...
export default function Home({ nav }: { nav: Nav }) {
  ...
  return <div className="home-screen">
    ...
    {upcoming.length ? <div className="stack-12">{upcoming.map(invite => <InviteCard key={invite.id} invite={invite} friends={friends} me={profile} compact onOpen={() => nav('invites')} />)}</div> :
